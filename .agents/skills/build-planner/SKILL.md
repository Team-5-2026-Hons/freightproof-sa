...
- This is not Codex's plan mode. Plan mode designs the next change inside one session and is the right tool for that; this skill writes the plan that outlives sessions — staged, on disk, executable by whoever turns up next. Use plan mode within a step; use this for the build.

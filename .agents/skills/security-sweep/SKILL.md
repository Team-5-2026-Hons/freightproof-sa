...
- Codex ships a built-in `/security-review` that checks the changes on your current branch — use that per change; use this sweep for the whole app, and for a report written for the app's owner rather than its developer. They complement each other, and neither replaces the other.

...
// Static side-effect import, as GeofenceMap does it: touches no `window`, so Next inlines it
// into this component's chunk. Leaflet's JS is loaded dynamically below.
...
/** Pin styling on the app's own tokens, not hex (no-raw-hex). Each severity also carries a
 *  glyph, so it never rests on colour alone. */
 ...
 *  type can't be read as markup. The link carries `returnTo`, so the report's Back button
 *  brings the dispatcher straight back here. */
...
/** The incident map. Leaflet's JS is imported inside an effect because it touches `window` at
 *  module scope, which would break the server render; a tile-server outage shows a message
 *  over the map instead of a grey void, and lifts on its own when tiles load again. */
export function IncidentMap({ pins, selection = null, returnTo }: IncidentMapProps) {
  ...
  useEffect(() => {
    ...
    async function init(): Promise<void> {
      try {
        ...
        // Scroll-wheel zoom starts off so the page scrolls past the map instead of zooming it;
        // it switches on once the dispatcher clicks into the map, off again on pointer-leave.
  ...
  // A row chosen in the table: bring the map into view, fly to that pin and open its popup,
  // as if it had been clicked on the map itself.
  useEffect(() => {
    const map = mapRef.current
  ...
  // `isolate` gives Leaflet's panes their own stacking context, so they never cover the
  // chart cards' "i" popovers.
  return (

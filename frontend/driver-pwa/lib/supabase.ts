...
//
// `||` rather than `??` deliberately: a developer whose .env.local copies .env.example
// verbatim has `NEXT_PUBLIC_SUPABASE_URL=`, i.e. the empty string. That is not nullish,
// so `??` would pass "" straight to createClient, which throws "supabaseUrl is required"
// and 500s every page. Empty must fall back to the placeholder just like unset does.

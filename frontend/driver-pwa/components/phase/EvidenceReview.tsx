...
export function EvidenceReview({ items }: EvidenceReviewProps) {
  return (
    <div className="flex flex-col gap-3 rounded-xl border border-outline-variant bg-surface-container-lowest p-4">
      <p className="text-lg font-semibold">Evidence collected</p>
      {items.map((item) => (
        <div key={item.label} className="flex flex-col gap-1">
          <p className="text-sm text-surface-on-variant">{item.label}</p>
          {item.isImage && item.value ? (
            // eslint-disable-next-line @next/next/no-img-element
            <img src={item.value} alt={item.label} className="max-h-24 w-full rounded-lg object-cover" />
          ) : item.value ? (
            <p className="text-base font-medium text-surface-on">{item.value}</p>
          ) : (
            <p className="flex items-center gap-1.5 text-base font-medium text-error">

...
export function Skeleton({ variant = 'text', lines = 1, className }: SkeletonProps) {
  if (variant === 'card') {
    return (
      <div className={cn('animate-pulse rounded-xl border-2 border-outline bg-surface-container-low p-4', className)}>
  ...
  if (variant === 'block') {
    return (
      <div className={cn('animate-pulse rounded-md bg-surface-container-high', className)} aria-hidden="true" />
    )
  }

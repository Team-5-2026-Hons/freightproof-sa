# FreightProof SA — Codex Instructions
...
**Codex may run:** `git status`, `git diff`, `git log --oneline -10`, `git branch --show-current`, `git fetch`, `git add <specific files>`.

**Codex must never run:** `git commit`, `git push`, `git merge`, `git rebase`, `git cherry-pick`, `git checkout <branch>`, `git stash`, `git reset`, `git restore`, or anything that modifies history or remote state.
...
- `AGENTS.md` (4-reviewer PR required)

...
| Runtime | React 19 (Server Components default; `"use client"` only at the lowest level needed) |
| Styling | Tailwind v3.4+ (`content`, not `purge`); design tokens via `tailwind.config.ts` |
| PWA shell | `next-pwa` with Workbox (driver-pwa only) |
...
├── public/
│   └── …                            # static images — but not icons (lucide-react)
...
| `PhotoCapture` | driver | The full-width camera tap-target per `DESIGN_SYSTEM.md §10.9`. On v1 it opens the device camera via `<input type="file" accept="image/*" capture="environment">`. Captures, shows a thumbnail, allows retake. |
...
- Photo capture pages use `<input type="file" accept="image/*" capture="environment">`. On v1 the file is held in `TripContext` for the demo; not uploaded.
...
#### H1 · Step 1 of 3 — Approach gate

| | |
|---|---|
| Route | `/trip/[id]/handshake/1/step/1-approach-gate` |
| User story | As a driver, I want to confirm I've arrived at the origin precinct so that the system knows to expect the next two evidence steps. |
| Purpose | Acknowledgement only — no input. |
| Layout | `StepHeader` · Big `MapPin` icon centred · `Card` showing precinct name, expected coordinates, current GPS (mocked to match) · Helper copy: "Wait for guard to scan vehicle disc, trailer disc, and your licence." · CTA "I'm at the gate · Continue" |
...
#### H4 · Step 1 of 3 — Approach destination gate

| | |
|---|---|
| Route | `/trip/[id]/handshake/4/step/1-approach-dest` |
| User story | As a driver, I want to confirm I've arrived at the destination precinct so that the system can prepare for the seal-intact verification. |
| Components | Same pattern as H1.1 / H3.1. |
...
6. `lib/hooks/*.ts` — required hooks.
7. `components/ui/*` — every primitive in §5.1.
8. `/_dev/tokens` page in both surfaces (visual sign-off that the design system is wired).
...
- **Push notifications** to driver. Phase 2.

...
**CI follow-up (2026-09-12, PR #51).** CI runs `mypy .` over the tests as well as the app.
Stage 1's unit-test helper `_pick` (in `tests/unit/test_exception_service.py`) built its
arguments as a `**dict`, which mypy rejects (6 `arg-type` errors). Stage 1's "mypy clean"
had only covered the app files. Fixed by giving `_pick` explicit typed parameters, in a
small follow-up commit on this branch. No behaviour change.

...
| D26 | **Tiles (2026-09-16): "Parcels complete" and "Receipts owed" are removed**, leaving four headline tiles: Live trips · Critical Exceptions Waiting · Licences & discs · Unused vehicles. Removed end to end: their two queries in `tiles.py`, the `parcels_complete` / `receipts_owed` response fields, their schemas and shared types, their copy, the `PARCEL_SHORTFALL_TYPES` constant (only the parcels tile used it) and the service's `anchored_phases` argument. The tile row is five columns wide at `xl` (three tiles plus the two-wide licence table). With these gone, no tile links into a tab any more; the `?tab=` address stays for the report Back link (D25). | Tom's call after using the page: he saw no use in either tile. Parcel shortfalls already show on the Problems tab (3.2 theft signs, 3.3 most common problems), and receipts were already dropped from the Evidence tab (D25). Removing the queries too, not just the tiles, means the tiles request does two fewer database round trips. |
| D27 | **Zoom (2026-09-16): every chart card gets a zoom-in button** after "Show table" (only when the chart is ready: not while loading, empty or failed). It opens the card's current view — legend, low-sample and caveat lines, and the chart or its table, whichever the card shows — in a modal at **80% of the viewport** over a **blurred page**, closed by the ×, a click on the background or Escape. Built once, as a universal piece: `components/analytics/fleet/ChartZoom.tsx` (the button, the modal and a context) used only by `ChartCard`, so every card gets it. Charts grow through the context: each wrapper asks `useChartHeight(normal)`, which returns the normal height on the card and a zoomed height (80% of the window less the modal chrome, at least 320 px, never smaller than normal) inside the modal. Horizontal bars (`CategoryBars` "bars") size by rows instead, 56 px each when zoomed. `components/ui/Modal.tsx` gained two optional props, `size="zoom"` and `backdrop="blur"`; every existing modal keeps its defaults. The zoomed map is its own Leaflet instance. | Tom's request, and his team's advice to make it one shared component. A context rather than a `zoomed` prop on every chart, so the tabs don't change and a future chart zooms by using the hook. The existing Modal already closes on Escape, on a real background click (not a drag that ends there, which matters for the map) and returns focus to the button, so it is reused rather than rebuilt. |
...
```
┌ Analytics ─────────────────────────────────────────────────────────────────────────────────┐
│ [Live trips 7] [Critical Exceptions Waiting 4 · oldest 2 d] [Licences & discs: 0 expired · 2 ≤30 d · …]     │
│ [Unused vehicles 2]                                     ← four tiles (D26), no filter                        │
├─────────────────────────────────────────────────────────────────────────────────────────────┤
...
- Tab choice is kept in the URL as `?tab=<id>` so a link can open a tab directly (e.g. Back from a report opened on
  the map, D25). No tile links into a tab since D26.
...
Four tiles since D26. The original **Parcels complete** tile (share of loaded closed trips in the last 30 days with no
parcel or waybill count mismatch, linking to Problems) and **Receipts owed** tile (attested anchored steps whose
blockchain receipt was pending or failed, linking to Evidence) were removed, queries included.
...
**5.4 Blockchain receipts — REMOVED (D25).** The Receipts owed tile was removed too (D26). Original design, kept
...
(5.6 was the **Parcels complete** tile — removed in D26.)
...
| `frontend/dispatcher/components/analytics/fleet/FleetTiles.tsx` | the headline tiles, four since D26 (new `FleetTile`; do **not** modify `components/ui/StatCard.tsx`) |
...
- **FP-154 anchoring and recovery (Chiko)** is now on `dev`. The Receipts owed tile and chart 5.4 read `anchor_status`
  either way (both since removed, D25 and D26). After merging, re-run `test_fleet_tiles.py` and the Stage 6 evidence tests.
...
| 9 Tile removal (D26) | DONE 2026-09-16 on `ui-changes` (Tom's browser check pending) | backend 1878 passed, 4 skipped, 0 failed / dispatcher 1003 passed (90 files); lint 0 errors; type-check clean. `ruff` and `mypy .` report errors only in code merged from `dev` since Stage 8, none in files this change touches: unused imports in migrations `2026_09_15_tim_close_blockchain_receipt_fk_drift.py` and `6ee6103df09a_merge_ciaran_and_tim_heads.py`; 39 mypy errors in `action_location_service.py`, `proximity_service.py`, `dev_truck_service.py` and their tests | Parcels complete and Receipts owed tiles removed end to end (see D26). `test_fleet_tiles.py`: their two tests deleted, the empty-org test asserts both fields are absent, the other-operator test no longer seeds a failed receipt. `FleetTiles.test.tsx`: their two tests deleted, a new test checks exactly four tiles. Comments naming the Receipts owed tile in `evidence.py`, `EvidenceTab.tsx` and `analytics/page.tsx` reworded. |
| 10 Chart zoom (D27) | DONE 2026-09-16 on `ui-changes` (Tom's browser check pending) | dispatcher 1017 passed (91 files; +14: `ChartZoom.test.tsx` 7, `ChartCard.test.tsx` 5, `Modal.test.tsx` 2); lint 0 errors; type-check clean. Backend untouched | New `ChartZoom.tsx` (`ChartZoom`, `useChartHeight`, `useIsChartZoomed`, `zoomChartHeight`). `ChartCard` renders one shared `view` for the card body and the modal. Height hook used by `TrendColumns`, `TrendLines`, `SplitColumns`, `LaneScatter`, `DonutChart`, `CategoryBars`, `PatternStrip` (`PatternColumns`) and `IncidentMap`. `Modal.tsx`: `size="zoom"` (`w-[80vw] h-[80dvh]`, body scrolls inside) and `backdrop="blur"` (`backdrop:backdrop-blur-sm`); the header and body now sit in one wrapper div so a zoomed body can fill the height. Button label "Zoom in: {title}" (lucide `ZoomIn`). |
